...
const TaskList = () => {
  ...
  const handleSelectFilterTemplate = (template) => {
    // Добавляем выбранный шаблон в список, если его еще нет
    if (!currentTask.filterTemplates.includes(template._id)) {
      setCurrentTask({
        ...currentTask,
        filterTemplates: [...currentTask.filterTemplates, template._id]
      });
    }
    setFilterTemplateDialogOpen(false);
  };
  
  const handleRemoveFilterTemplate = (templateId) => {
    setCurrentTask({
      ...currentTask,
      filterTemplates: currentTask.filterTemplates.filter(id => id !== templateId)
    });
  };
  
  const getTemplateNames = () => {
    return currentTask.filterTemplates.map(id => {
      const template = filterTemplates.find(t => t._id === id);
      return template ? template.name : 'Неизвестный шаблон';
    });
  };
        ...
        <DialogContent>
          ...
          <Box sx={{ mb: 2 }}>
            <Paper sx={{ p: 2 }}>
              ...
              {currentTask.filterTemplates.length > 0 ? (
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                  {getTemplateNames().map((name, index) => (
                    <Chip
                      key={index}
                      label={name}
                      onDelete={() => handleRemoveFilterTemplate(currentTask.filterTemplates[index])}
                      color="primary"
                    />
                  ))}
                </Box>
                ...
                <Alert severity="info">
                  Не выбраны шаблоны фильтрации. Все посты будут обработаны без фильтрации.
                </Alert>
              )}
            </Paper>
          </Box>
          ...
          <FilterTemplateManager
            ...
            onSelect={handleSelectFilterTemplate}
          />
        </DialogContent>
        <DialogActions>
